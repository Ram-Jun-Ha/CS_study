...
제4정규화를 거치기 전의 다치 종속성 문제를 발생하지 않지만, 중복된 데이터가 삽입되는 이상현상이 발생하게 된다. 제5정규화는 **비손실 분해**를 더이상 거치지 않도록 이뤄져야 한다. 해당 테이블을 분리하게 되면 중복된 데이터를 없애는 것이기 때문에 비손실 분해에 해당하므로 곧 제5정규화를 거친다고 볼 수 있다.
