...
즉, A → B 인 의존성에서 단일 값 A와 다중 값 B가 존재한다면 다치 종속이라고 할 수 있다. 이러한 종속을 이중 화살표를 사용해서 A ↠ B 로 표기한다.

다치 종속은 최소 2개의 컬럼이 다른 컬럼에 종속되어야 하기 때문에 최소 3개의 컬럼이 필요하다.

<p align="center">
<img src="./img/스크린샷 2024-05-21 오후 6.30.11.png" alt="img1" width="300"/>
</p>

위의 테이블은 자격증이 개발자에 의존하고, 언어도 개발자에 의존한다. 문제는, 홍길동은 빅데이터 분석기사이지만 C++뿐만이 아닌, C언어도 할 줄 알고, 홍길동은 C++를 할 줄 알지만, 정보처리기사뿐만이 아닌 빅데이터 분석기사이기도 하다는 점이다.

<p align="center">
<img src="./img/스크린샷 2024-05-21 오후 6.33.44.png" alt="img1" width="300"/>
</p>

테이블 분리를 통해 다치 종속을 없앰으로써 개발자가 어떤 언어를 할 수 있는지를 표현하고 개발자가 어떤 자격증을 지니고 있는지를 명확하게 나타낼 수 있게 된다.
...
하나의 릴레이션을 여러개의 릴레이션으로 분해하였다가, 다시 조인했을 때 데이터 손실이 없고 필요없는 데이터가 생기는 것을 말한다. 조인 종속성은 다치 종속의 개념을 더 일반화한 것이다. 만약 위의 제4정규화를 거친 테이블을 조인하게 되면 아래와 같이 나타난다.

<p align="center">
<img src="./img/스크린샷 2024-05-21 오후 6.38.08.png" alt="img1" width="300"/>
</p>

제4정규화를 거치기 전의 다치 종속성 문제가 발생하지 않지만, 중복된 데이터가 삽입되는 이상현상이 발생하게 된다. 제5정규화는 **비손실 분해**를 더이상 거치지 않도록 이뤄져야 한다. 해당 테이블을 분리하게 되면 중복된 데이터를 없애는 것이기 때문에 비손실 분해에 해당하므로 곧 제5정규화를 거친다고 볼 수 있다.
